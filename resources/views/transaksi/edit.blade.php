                ...
                @else
                    <form method="POST" action="{{ url('/transaksi/' . $transaksi->penjualan_id) }}" class="form-horizontal">
                                ...
                                <select class="form-control" id="user_id" name="user_id" required
                                    ...
                                    @foreach ($user as $item)
                                        <option value="{{ $item->user_id }}"
                                            {{ $item->user_id == $transaksi->user_id ? 'selected' : '' }}>{{ $item->username }}
                                        </option>
                                    @endforeach
                                </select>
                                @error('username')
                            ...
                            @foreach ($transaksi->detail_penjualan as $key => $detail)
                                <div id="row{{ $key + 1 }}">
                                    <div class="form-group row">
                                        ...
                                        <div class="col-10">
                                            <select class="form-control" id="barang_id{{ $key + 1 }}" name="barang_id[]"
                                                required>
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-10">
                                            <input type="number" class="form-control" id="jumlah{{ $key + 1 }}"
                                                name="jumlah[]" value="{{ $detail->jumlah }}" required>
                                            @error('jumlah')
                                                <small class="form-text text-danger">{{ $message }}</small>
                                            @enderror
                                        </div>
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-10">
                                            <button type="button" class="btn btn-danger btn-sm"
                                                onclick="removeData({{ $key + 1 }})">Hapus</button>
                                        </div>
                                    </div>
                                </div>
                            @endforeach
                        ...
                        <div class="form-group row">
                            <label class="col-2 control-label col-form-label"></label>
                            <div class="col-10">
                                <button type="button" class="btn btn-primary btn-sm" id="add" onclick="addData()">Tambah
                                    Barang</button>
                            </div>
                        </div>
                        <div class="form-group row">
